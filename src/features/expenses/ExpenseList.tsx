...
const ExpenseList: React.FC = () => {
  const [filters, setFilters] = useState<{
    ...
    dateRange: [moment().startOf('month'), moment().endOf('month')] // Default to current month
  });
  ...
  const handleRemove = async (id: string) => {
    try {
      await removeExpense({
        variables: { id },
        refetchQueries: [{ query: GET_EXPENSES, variables: { /* pass current filters */ } }],
      });
  ...
  const columns = [
    ...
    { title: 'Amount', dataIndex: 'amount', key: 'amount', render: (amount: number) => `$${amount.toFixed(2)}`, align: 'right' as 'right', sorter: (a: ExpenseData, b: ExpenseData) => a.amount - b.amount },
  ...
  return (
    <div>
            ...
            <Row gutter={16}>
                <Col xs={24} sm={12} md={8}>
                    <Select placeholder="Filter by Category" style={{ width: '100%' }} onChange={(val) => handleFilterChange('categoryId', val)} loading={categoriesLoading} allowClear>
                        {categoriesData?.expenseCategories.map((cat: { id: unknown; name: string | number | bigint | boolean | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<string | number | bigint | boolean | React.ReactPortal | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | null | undefined> | null | undefined; }) => <Option key={cat.id} value={cat.id}>{cat.name}</Option>)}
                    </Select>
                </Col>
                ...
                <Col xs={24} sm={12} md={6}>
                    <Space>
                        ...
                        <Button icon={<ReloadOutlined />} onClick={() => setFilters({dateRange: [moment().startOf('month'), moment().endOf('month')]})}>Reset</Button>
                    </Space>
                </Col>
            </Row>
        </Card>
        ...
        <Card style={{ marginBottom: 16 }}>
            <Statistic title="Total Expenses (Filtered Period)" value={totalExpenses} precision={2} prefix="$" loading={loading} />
        </Card>
